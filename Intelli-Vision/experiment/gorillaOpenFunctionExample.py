...
def get_prompt(user_query: str, func_name: list = []) -> str:
    """Generates a conversation prompt based on the user's query and a list of functions."""
    if len(func_name) == 0:
        return f"USER: <<question>> {user_query}\\nASSISTANT: "
    functions_string = json.dumps(func_name)
...
functions = [
    {
        "name": "Uber Carpool",
        "api_name": "uber.ride",
        "description": "Find suitable ride for customers given the location, type of ride, and the amount of time the "
                       "customer is willing to wait as parameters",
        "parameters": [
...
prompt = get_prompt(query, func_name=functions)
output = pipe(prompt)
print(output)
